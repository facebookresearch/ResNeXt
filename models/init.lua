...
--
--  Generic model creating code. For the specific ResNeXt model see
--  models/resnext.lua
--
...
function M.setup(opt, checkpoint)
   ...
   -- This is useful for fitting ResNet/ResNeXt-50 on 4 GPUs, but requires that all
...
function M.shareGradInput(model)
      ...
      local cache = {}
      local input_sign_cache = {}
      model:apply(function(m)
         local moduleType = torch.type(m)
         if torch.isTensor(m.gradInput) and moduleType ~= 'nn.ConcatTable' and moduleType ~= 'nn.JoinTable' then
      ...
      for i, m in ipairs(model:findModules('nn.ConcatTable')) do
         if cache[i % 3] == nil then
            cache[i % 3] = torch.CudaStorage(1)
         end
         m.gradInput = torch.CudaTensor(cache[i % 3], 1, 0)
      end
end
